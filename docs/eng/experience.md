...
STYNX PDF evidence block, not a real CMS/PKCS#7/PAdES signer. eSocial,
...
**Engine de geração de PDFs:** `sgp-report-service` usando **Carbone** (templates DOCX/ODS compilados para PDF via LibreOffice headless) ou biblioteca PDF dedicada para layouts programáticos de baixa complexidade. Contracheque oficial e comprovante anual usam `@stynx/pdf/public-payroll` por meio do adaptador `PdfABuilderService`, com metadados PDF/A-1b-style, hash SHA-256 e retenção registrada em `public.generated_report_file`. Arquivos TXT/XML gerados por builders TypeScript tipados em `sgp-integrations-worker`.
...
| **Template**              | `backend/src/report-service/payslip/payslip-template.ts`; engine `@stynx/pdf/public-payroll` via `PdfABuilderService`                                                                                                                                                                                                                                                        |
...
| **Assinatura digital**    | PDF/A emitido com bloco interno de evidência por `@stynx/pdf/evidence`; `public.generated_report_file.signature_kind` e `signed_at` gravados na persistência oficial. PAdES legal real permanece decisão pendente.                                                                                                                                                           |
...
| **Evidência de paridade** | Arquivo binário inicia com `%PDF-`, metadados PDF/A-1b e bloco `%%STYNX-PADES-SIGNATURE` presentes, golden PDF byte-estável em `tests/backend/golden/payslip-pdf-a-v01/`, `file_hash` SHA-256 persistido, comparação de matrícula, competência, códigos de verba, valores brutos, descontos, líquido, total de proventos e total de descontos com tolerância zero em valores |
...
| **Template**              | `backend/src/report-service/yearly-income/yearly-income-template.ts`; engine `@stynx/pdf/public-payroll` via `PdfABuilderService`                                                                                                  |
...
| **Assinatura digital**    | PDF/A emitido com bloco interno de evidência por `@stynx/pdf/evidence`; hash SHA-256 e `signature_kind` persistidos em `public.generated_report_file`                                                                              |
...
| **Evidência de paridade** | PDF inicia com `%PDF-`, metadados PDF/A-1b e bloco `%%STYNX-PADES-SIGNATURE` presentes, hash persistido, empregado autenticado só baixa o próprio comprovante, `taxable_total + exempt_total` e IRRF reconciliam com totais S-1210 |
