...
| **Assinatura digital**    | PDF/A emitido com bloco PAdES pelo `PadesAdapter`; `public.generated_report_file.signature_kind = ICP_BRASIL_A1` e `signed_at` gravados na persistência oficial                                                                                                                                                                                          |
...
| **Evidência de paridade** | Arquivo binário inicia com `%PDF-`, metadados PDF/A-1b e bloco PAdES presentes, golden PDF byte-estável em `tests/backend/golden/payslip-pdf-a-v01/`, `file_hash` SHA-256 persistido, comparação de matrícula, competência, códigos de verba, valores brutos, descontos, líquido, total de proventos e total de descontos com tolerância zero em valores |
...
| **Assinatura digital**    | PDF/A emitido com bloco PAdES pelo `PadesAdapter`; hash SHA-256 e `signature_kind` persistidos em `public.generated_report_file`                                                                                 |
...
| **Evidência de paridade** | PDF inicia com `%PDF-`, metadados PDF/A-1b e bloco PAdES presentes, hash persistido, empregado autenticado só baixa o próprio comprovante, `taxable_total + exempt_total` e IRRF reconciliam com totais S-1210   |
