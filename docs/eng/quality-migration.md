...
No pacote frontend, `npm run test:frontend:coverage` é o gate canônico de cobertura Angular para superfícies frontend que permanecem sob escopo SGP. O gate agregado executa o ratchet do `sgp-portal` com Vitest/V8, reporters `text-summary`, `json-summary` e `lcov`, e limiares em `frontend/angular.json`. Superfícies administrativas pertencem ao Stynx; `test:admin:coverage` pode continuar executável como sinal de transição, mas não conta como evidência de entrega SGP. O root declara `jsdom` como dependência de desenvolvimento porque o workspace npm hoista o Vitest usado pelo builder Angular para a raiz; sem essa resolução explícita, o ambiente jsdom falha de forma dependente do layout de `node_modules`.
...
O enforcement corrente é orquestrado pelo `scripts/run.mjs` a partir da raiz do workspace. O script backend `test:cov` executa Jest, inclui specs unitários e e2e cobertos, aplica `coverageThreshold.global` de 85 % para `lines`, `branches` e `functions`, e gera `lcov`, `text-summary` e `cobertura`. O script frontend `test:frontend:coverage` executa o target Angular coberto sob escopo SGP e grava artefatos em `frontend/coverage/sgp-portal/`; admin é superfície Stynx e não é bloqueador SGP. Não há `jest.config.ts` raiz nem thresholds por `projects` no pacote atual; qualquer corte backend por módulo deve ser adicionado em decisão futura antes de ser tratado como gate.
