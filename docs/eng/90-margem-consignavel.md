...
A regra padrao segue a Lei 14.131/2021: margem total de 40% sobre a base liquida, segregada em 35% para consignados gerais e 5% exclusivo para cartao consignado/saque. O Decreto 8.690/2016, o Decreto 11.150/2022 e a Lei 8.112/1990 art. 45 orientam a ordem de descontos e o respeito ao minimo disponivel.
...
- `consignment.margin.card_pct`: percentual exclusivo para cartao consignado/saque, padrao `0.05`.
...
- `available_card = max(round(net_base * card_pct, 2) - used_card, 0)`

`used_general` soma contratos ativos `PAYROLL_LOAN` e `OTHER`; `used_card` soma contratos ativos `CARD`. A criacao de averbação recusa valor de parcela maior que a margem disponivel do respectivo grupo com resposta HTTP 422.
