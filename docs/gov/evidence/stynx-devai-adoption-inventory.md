...
| ---------------------------------------- | --------------------------------------------------------------------------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------ |
| DB/RLS and tenancy                       | `backend/src/database/`, `database/sql/`, `tests/rls/`                                  | declared data/tenancy packages are not imported                          | tenant context is required for queries; canonical RLS/alignment gates                |
| Auth/authz/context                       | `backend/src/auth/`, `backend/src/iam/`, `backend/src/common/request-context/`          | auth, backend and contracts are active                                   | global positive/negative auth, permission metadata, actor/request/tenant propagation |
| Sessions                                 | `backend/src/auth/session/`                                                             | backend package not composed; frontend uses angular-auth session service | authenticated/anonymous session DTO and browser token lifecycle                      |
| Audit                                    | `backend/src/audit/`, `backend/src/common/audit/`                                       | audit package declared but not imported                                  | append-only/redacted actor, tenant and request metadata; required mutation audit     |
| Storage                                  | `backend/src/documents/`                                                                | storage package declared but not imported                                | authorized S3 presign, key policy, registration and audit behavior                   |
| Logging/privacy                          | `backend/src/common/logging/`, `backend/src/common/observability/`, `docs/gov/privacy/` | packages declared but not imported                                       | structured request/trace fields and governed PII redaction                           |
| Health                                   | `backend/src/health/`, `backend/src/common/bootstrap/worker-readiness-probe.ts`         | health package declared but not imported                                 | API liveness/readiness plus worker probe and eight-runtime topology                  |
...
| DEVAI                                    | `.devai/config/` and retained `.devai/state/`                                           | CLI absent                                                               | existing config validates, but no live inventory/sensor/scorecard pipeline yet       |
