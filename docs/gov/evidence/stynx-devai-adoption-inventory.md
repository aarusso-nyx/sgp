...
| Logging/privacy                          | `backend/src/stynx/`, `backend/src/common/observability/`, `docs/gov/privacy/`  | STYNX logger active with SGP redaction policy; privacy mutation runtime intentionally disabled                                                          | structured request/trace fields and governed PII redaction                           |
...
| Rate limit                               | `backend/src/common/rate-limit/` and runtime bootstraps                         | STYNX default disabled; SGP dual IP/tenant bucket policy retained                                                                                       | configured route limits and `429` behavior                                           |
| Idempotency                              | `backend/src/common/idempotency/`, payroll and worker idempotency paths         | STYNX default disabled; SGP durable replay/lease adapter retained                                                                                       | tenant-scoped reserve/replay/conflict and deterministic worker keys                  |
| i18n                                     | backend messages and `frontend/src/app/core/i18n/`                              | STYNX runtime `pt-BR` catalog active alongside Angular compile-time product messages                                                                    | `pt-BR` product messages and frontend extraction/check                               |
| Frontend auth/tenant                     | `frontend/src/app/shared/stynx-runtime-config.ts`, guards in both apps          | STYNX Angular foundation, auth and tenancy own bearer, request-ID, error and tenant HTTP plumbing                                                       | login callback, token injection, permissions and tenant switching                    |
| Frontend UI/storage/profile/sessions/SDK | Admin/Portal feature and API layers                                             | STYNX UI available and multipart upload executor composed; generated SGP clients retained; profile and active-session providers await matching SGP APIs | unit suites, generated clients and Playwright journeys                               |
