...
**Skill awareness.** Invoke DEVAI canonical `SKILL-round-verify-publish` for verification only, with SGP repository configuration from `.devai/config/project.json`; publish remains disabled without explicit user authorization.
