...
status is tracked in `docs/gov/evidence/implementation-status.md` and should be
...
- `docs/gov/evidence/implementation-status.md` is the current status document.
...
- `docs/gov/generated/database/alignment-matrix.json` has no in-scope explicit exclusions left; `dbo.sysdiagrams` remains the only approved technical out-of-scope object unless `docs/eng/` explicitly approves another exclusion.
...
- Backend coverage thresholds are configured to match `docs/eng/quality-migration.md`.
