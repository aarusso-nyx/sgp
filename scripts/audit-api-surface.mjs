...
const usage = `
Usage: node scripts/audit-api-surface.mjs [--round <n>] [--dry-run] [--output-root <path>] [--repo-root <path>]

Render docs/eng/69-api-route-alignment.json into an audit API surface digest and run API drift checks when present.
`;
...
export async function buildApiSurface(repoRoot, round) {
  const alignmentPath = join(repoRoot, 'docs', 'eng', '69-api-route-alignment.json');
  ...
  return {
    generated_by: 'scripts/audit-api-surface.mjs',
    round,
    source: 'docs/eng/69-api-route-alignment.json',
...
async function runChecks(repoRoot) {
  const checks = [];
  const alignmentScript = join(repoRoot, 'scripts', 'check-api-route-alignment.mjs');
  const operationScript = join(repoRoot, 'scripts', 'check-api-operation-decorators.mjs');
  if (await exists(alignmentScript)) {
    const result = await runNodeScript(repoRoot, alignmentScript, ['--json']);
    checks.push({
      name: 'check-api-route-alignment',
      ok: result.ok,
  ...
  if (await exists(operationScript)) {
    const result = await runNodeScript(repoRoot, operationScript);
    checks.push({
      name: 'check-api-operation-decorators',
      ok: result.ok,
      status: result.status,
