...
import { optionValue } from '../../cli.mjs';
...
function parseOption(name) {
  return optionValue(process.argv.slice(2), name);
}
...
function collectAddedLineFindings(diffText) {
  ...
  for (const line of diffText.split('\n')) {
    ...
    if (
      addedLine.includes(forbiddenCommand) &&
      !isAllowedHistoricalReference(currentFile, addedLine)
    ) {
      findings.push({ file: currentFile, line: addedLine });
    }
  }
...
function isAllowedHistoricalReference(file, line) {
  return file === 'docs/gov/audit/backlog-ledger.md' && line.trim().startsWith('| R2-04 ');
}
