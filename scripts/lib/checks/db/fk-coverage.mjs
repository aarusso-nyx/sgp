...
function inferredForeignKeyKey(foreignKey) {
  return `${foreignKey.schema}.${foreignKey.table}.${foreignKey.column}->${foreignKey.targetSchema}.${foreignKey.targetTable}.${foreignKey.targetColumn}`;
}

function inferredForeignKeyValidationClause(foreignKey) {
  return promotedInferredForeignKeys.has(inferredForeignKeyKey(foreignKey)) ? '' : ' NOT VALID';
}
...
function renderMissingFkSql(missingForeignKeys) {
  ...
  for (const foreignKey of missingForeignKeys) {
    ...
    const validationClause = inferredForeignKeyValidationClause(foreignKey);
    lines.push(
      ...
      `    FOREIGN KEY (${foreignKey.column}) REFERENCES ${foreignKey.targetSchema}.${foreignKey.targetTable}(${foreignKey.targetColumn})${deleteClause}${validationClause};`,
      '',
    );
  }
  return lines.join('\n');
