CREATE FUNCTION avaliacao.apply_merit_progression() RETURNS trigger
    LANGUAGE plpgsql
    AS $$
DECLARE
...
CREATE FUNCTION avaliacao.fn_get_vencimento_vigente(p_salary_range_level_id uuid, p_competencia date) RETURNS numeric
    LANGUAGE sql STABLE
    AS $$
  SELECT COALESCE(
    (
