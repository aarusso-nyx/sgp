    ...
    for (const filePath of controllerFiles(srcDir)) {
      ...
      const visit = (node: ts.Node) => {
        if (ts.isMethodDeclaration(node)) {
          ...
          const isRoute = methodDecorators.some((name) => HTTP_DECORATORS.has(name));
          const isCovered =
          ...
          if (isRoute && !isCovered) {
            missing.push(`${filePath.replace(`${srcDir}/`, '')}:${node.name.getText(sourceFile)}`);
          }
        }
    ...
    for (const filePath of controllerFiles(srcDir)) {
      ...
      const visit = (node: ts.Node) => {
        if (ts.isMethodDeclaration(node)) {
          const methodDecorators = decoratorNames(node, sourceFile);
          const classDecorators = decoratorNames(node.parent, sourceFile);
          const isMutating = methodDecorators.some((name) => MUTATING_DECORATORS.has(name));
          const hasPermission =
            methodDecorators.includes('RequirePermission') ||
            classDecorators.includes('RequirePermission');
          if (isMutating && !hasPermission && !methodDecorators.includes('Public')) {
            missing.push(`${filePath.replace(`${srcDir}/`, '')}:${node.name.getText(sourceFile)}`);
          }
        }
        ts.forEachChild(node, visit);
