...
function readSpoolSql(): string {
  return readFileSync(resolve(workspaceRoot, 'database/sql/16-esocial-spool.sql'), 'utf8');
}

describe('public.esocial_spool RLS contract', () => {
  ...
  it('declares the SGP-side canonical spool table without cross-boundary stynx FKs', () => {
    expect(spoolSql).toContain('CREATE TABLE public.esocial_spool');
    expect(spoolSql).toContain('CONSTRAINT esocial_spool_tenant_fkey');
    expect(spoolSql).not.toMatch(/REFERENCES\s+esocial\./i);
    expect(spoolSql).not.toMatch(/FOREIGN\s+DATA\s+WRAPPER/i);
  });

  it('forces tenant RLS and the esocial_spool_tenant_isolation policy', () => {
    expect(sql).toContain('ALTER TABLE public.esocial_spool FORCE ROW LEVEL SECURITY');
    expect(sql).toContain('CREATE POLICY esocial_spool_tenant_isolation ON public.esocial_spool');
  ...
  it('keeps the idempotency, lookup, GIN, and LGPD catalog surfaces', () => {
    expect(sql).toContain('esocial_spool_active_payload_hash_uidx');
    expect(sql).toContain('esocial_spool_tenant_status_created_idx');
    expect(sql).toContain('esocial_spool_kind_event_created_idx');
    expect(sql).toContain('esocial_spool_source_ref_gin_idx');
    expect(sql).toContain('sgp_apply_esocial_spool_pii_comments');
