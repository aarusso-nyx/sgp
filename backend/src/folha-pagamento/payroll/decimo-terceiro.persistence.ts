...
export interface DecimoCalculationRow extends QueryResultRow {
  avos: number;
  base: string;
...
export class DecimoTerceiroPersistence {
  async ensureCatalog(
    client: PoolClient,
    kind: DecimoKind,
  ): Promise<CatalogRow> {
  ...
  async computeEmployeeCalculation(
    client: PoolClient,
    employmentLinkId: string,
    kind: DecimoKind,
    year: number,
  ): Promise<DecimoCalculationRow | undefined> {
    const calcRows = await client.query<DecimoCalculationRow>(
      ...
      `,
      [employmentLinkId, kind, year],
    );
    return calcRows.rows[0];
  }

  async insertItem(client: PoolClient, input: PayrollItemInput): Promise<void> {
    await client.query(
      `
