...
export interface ServidorImportAcceptedRow {
  rowNumber: number;
...
export interface ServidorImportRejectedRow {
  rowNumber: number;
  message: string;
}

export interface ServidorImportResult {
  payrollRunId: string;
  fileName: string;
  fileHash: string;
  totalRows: number;
  acceptedRows: number;
  rejectedRows: number;
  accepted: ServidorImportAcceptedRow[];
  errors: ServidorImportRejectedRow[];
}
...
export class ServidorImportService {
  ...
  async importFile(
    ...
    const fileName = file.originalname ?? 'servidor-import.xlsx';
  ...
  private async persistRows(
    ...
    const accepted: ServidorImportAcceptedRow[] = [];
    const errors: ServidorImportRejectedRow[] = [];
  ...
  private async upsertImportedItem(
    ...
    const result = await client.query<ImportedItemRow>(
      ...
      [
        ...
        row.referenceValue ?? '',
        row.amount,
        row.notes || `Servidor XLSX import row ${row.rowNumber}`,
      ],
    );
