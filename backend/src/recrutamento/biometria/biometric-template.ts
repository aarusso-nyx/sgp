...
export interface ExtractedBiometricTemplate {
  template: Buffer;
  qualityScore: string;
}

export function extractBiometricTemplate(
  kind: BiometricKind,
  sampleBase64: string,
  ...
  return {
    template: normalized.subarray(0, kind === 'FACE' ? 64 : 48),
    qualityScore: quality.toFixed(6),
  };
}

export function scoreBiometricTemplates(left: Buffer, right: Buffer): number {
...
export function encryptTemplate(template: Buffer, kmsKeyId: string): Buffer {
