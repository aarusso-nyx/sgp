...
const ESOCIAL_SPOOL_PERMISSIONS = [
  'esocial.event.read',
  'esocial.event.write',
] as const;

const ESOCIAL_SPOOL_RETURNING = `
  message_id,
  tenant_id,
  kind,
  event_class,
...
@Injectable()
export class EsocialSpoolService {
  constructor(private readonly databaseService: DatabaseService) {}

  async recordPending(input: RecordPendingInput): Promise<EsocialSpoolRecord> {
    const payloadHash = input.payloadHash ?? sha256Json(input.payload);
    return this.runForTenant(input.tenantId, async () => {
      const [existing] = await this.databaseService.query<EsocialSpoolRow>(
        `
        SELECT ${ESOCIAL_SPOOL_RETURNING}
        FROM public.esocial_spool
        WHERE tenant_id = $1::uuid
          AND kind = $2
          AND event_class = $3
          AND payload_hash = $4
          AND status NOT IN ('REJECTED'::public.esocial_spool_status, 'DLQ'::public.esocial_spool_status)
      ...
      const [created] = await this.databaseService.query<EsocialSpoolRow>(
        `
        INSERT INTO public.esocial_spool (
          message_id,
          tenant_id,
          kind,
          event_class,
        ...
        RETURNING ${ESOCIAL_SPOOL_RETURNING}
        `,
        [
  ...
  async recordSent(input: RecordSentInput): Promise<EsocialSpoolRecord> {
    return this.runForTenant(input.tenantId, async () => {
      const [updated] = await this.databaseService.query<EsocialSpoolRow>(
        `
        UPDATE public.esocial_spool
        SET
          status = 'SENT'::public.esocial_spool_status,
  ...
  async recordResponse(
    ...
    return this.runForTenant(input.tenantId, async () => {
      const [updated] = await this.databaseService.query<EsocialSpoolRow>(
        ...
        UPDATE public.esocial_spool
        SET
          status = $3::public.esocial_spool_status,
          response = $4::jsonb,
          response_hash = $5,
          tstamp_recv = COALESCE(tstamp_recv, now()),
          tstamp_terminal = CASE
            WHEN $3::public.esocial_spool_status IN ('ACCEPTED', 'REJECTED') THEN COALESCE(tstamp_terminal, now())
  ...
  async recordError(input: RecordErrorInput): Promise<EsocialSpoolRecord> {
    ...
    return this.runForTenant(input.tenantId, async () => {
      const [updated] = await this.databaseService.query<EsocialSpoolRow>(
        ...
        SET
          ...
          END,
          tstamp_terminal = CASE
            WHEN $3::public.esocial_spool_status IN ('REJECTED', 'DLQ') THEN COALESCE(tstamp_terminal, now())
            ELSE tstamp_terminal
          END
        WHERE tenant_id = $1::uuid
          AND message_id = $2::uuid
  ...
  async findById(
    tenantId: string,
    messageId: string,
  ): Promise<EsocialSpoolRecord | null> {
    return this.runForTenant(tenantId, async () => {
      const [row] = await this.databaseService.query<EsocialSpoolRow>(
        `
        SELECT ${ESOCIAL_SPOOL_RETURNING}
        FROM public.esocial_spool
        WHERE tenant_id = $1::uuid
  ...
  async findByTenant(
    tenantId: string,
    filters: EsocialSpoolListFilters = {},
  ): Promise<EsocialSpoolRecord[]> {
    return this.runForTenant(tenantId, async () => {
      const rows = await this.databaseService.query<EsocialSpoolRow>(
        `
        SELECT ${ESOCIAL_SPOOL_RETURNING}
        FROM public.esocial_spool
        WHERE tenant_id = $1::uuid
          AND ($2::public.esocial_spool_status IS NULL OR status = $2::public.esocial_spool_status)
...
function mapRow(row: EsocialSpoolRow): EsocialSpoolRecord {
  return {
