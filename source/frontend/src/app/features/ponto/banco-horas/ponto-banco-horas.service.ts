...
export class PontoBancoHorasService {
  ...
  movements(hourBankId: string): Observable<HourBankMovement[]> {
    return this.http.get<HourBankMovement[]>(
      `/api/v1/ponto/banco-horas/${hourBankId}/movimentos`,
    );
  }
  ...
  adjust(payload: {
    ...
    return this.http.post<HourBankMovement>(
      '/api/v1/ponto/banco-horas/ajuste-manual',
      payload,
    );
  }
}
