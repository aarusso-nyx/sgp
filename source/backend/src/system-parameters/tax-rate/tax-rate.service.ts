...
export class TaxRateService {
  ...
  validateContinuity(brackets: UpsertIrrfTaxRateTableDto['brackets']): void {
    ...
    if (!new Decimal(sorted[0].bracketMin).equals(0)) {
      throw new BadRequestException('First IRRF bracket must start at 0.00');
    }
    for (let index = 0; index < sorted.length; index += 1) {
      ...
      if (max && max.lessThan(min)) {
        throw new BadRequestException(
          'IRRF bracket max must be greater than min',
        );
      }
      if (index < sorted.length - 1) {
        if (!max) {
          throw new BadRequestException(
            'Only the final IRRF bracket can be open-ended',
          );
        }
        const next = new Decimal(sorted[index + 1].bracketMin);
        if (!next.equals(max.plus('0.01'))) {
          throw new BadRequestException(
            'IRRF brackets must be continuous by cent',
          );
        }
      } else if (max) {
        throw new BadRequestException('Final IRRF bracket must be open-ended');
      }
    }
  }
  ...
  async upsertIrrfTable(
    ...
    this.ensureDatabase();
    this.validateContinuity(input.brackets);
    ...
    await this.databaseService.transaction(async (client) => {
      ...
      for (const [index, bracket] of input.brackets.entries()) {
        await client.query(
          ...
          VALUES (
            public.sgp_current_tenant_uuid(), $1, $2, $3, 'IRRF', $4,
            $5::numeric(18, 6), 'IRRF', $6::date, $7::date, $8::numeric(14, 2),
          ...
          [
            bracket.code,
            `IRRF ${referenceYear} faixa ${index + 1}`,
            ...
            JSON.stringify({ importedBy: 'admin-irrf-table' }),
          ],
        );
      }
      await this.appendAudit(client, competenceStart, referenceYear);
    });

    return this.listIrrfTables(competenceStart);
  }
  ...
  private async appendAudit(
    ...
    await client.query(
      ...
      SELECT public.sgp_append_audit_event(
        ...
        jsonb_build_object('event', 'system.tax_rate.irrf.upserted', 'referenceYear', $2),
        NULL::text, NULL::text, NULL::text
      )
      `,
      [competenceStart, referenceYear],
    );
