...
SET formula_function_ddl = 'CREATE OR REPLACE FUNCTION payroll_calc.f_rpps_progressive(uuid, integer, integer) RETURNS numeric LANGUAGE sql VOLATILE SECURITY DEFINER SET search_path = payroll_calc, hr, payroll, public, pg_catalog AS $function$ SELECT payroll_calc.compute_rpps(employee.tenant_id, employee.employment_link_id, payroll_calc.base_rpps(employee.id, make_date($3, $2, 1)), make_date($3, $2, 1)) FROM hr.employee employee WHERE employee.id = $1; $function$;',
    formula_ready = true,
    formula_error = NULL,
    updated_at = now()
WHERE formula_alias = 'rpps'
   OR formula_function_name = 'f_rpps_progressive';
